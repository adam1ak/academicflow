import { SubjectDetailResponse, SubjectScheduleItem } from "../../../types/plan"
...
export interface ComputeAlertsParams {
    ...
    activeSchedule: SubjectScheduleItem[]
    maxConcurrent: number
    startDate?: string
}

export function computeAlerts(params: ComputeAlertsParams): AlertItem[] {
    const { subjects, deadlines, activeSchedule, maxConcurrent, startDate } = params
    ...
    if (startDate) {
        const planStartMs = new Date(startDate).getTime()
